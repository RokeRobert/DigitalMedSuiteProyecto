...
const citasPorFiltro = {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!seccionCitas) return; // Evita errores si no es la sección actual
    ...
    if (btnBuscar) {
      btnBuscar.addEventListener('click', buscarCita);
        ...
        // Activar el filtro de búsqueda (Paciente / Médico)
    let tipoBusqueda = 'paciente'; // por defecto
    ...
    tipoBusqueda = filtro.getAttribute('data-filtro');
        ...
        // También activar búsqueda con la tecla Enter
        const inputBusqueda = document.getElementById('busquedaCita');
        if (inputBusqueda) {
        inputBusqueda.addEventListener('keypress', function (e) {
        if (e.key === 'Enter') {
        e.preventDefault(); // Evita recargar la página si está en un formulario
         buscarCita();       // Ejecuta la función de búsqueda
        }
        });
        }

    // Cargar por defecto las citas de hoy
    cargarCitas('hoy');
  });
